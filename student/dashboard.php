...
// Next exam (REAL SYSTEM using exam_date)
$nextTestQuery = $conn->query("SELECT * FROM tests ORDER BY exam_date ASC LIMIT 1");
$nextTest = $nextTestQuery->num_rows > 0 ? $nextTestQuery->fetch_assoc() : null;
...
<!-- ================= PDF DOWNLOAD ================= -->
<a href="export_result.php" class="btn btn-danger mb-3">
...
// ================= COUNTDOWN =================
<?php if ($nextTest && !empty($nextTest['exam_date'])): ?>

const examDate = new Date("<?= $nextTest['exam_date'] ?>").getTime();
